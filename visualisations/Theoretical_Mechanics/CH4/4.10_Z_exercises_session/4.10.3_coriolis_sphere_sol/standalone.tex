% This figure was made starting from a figure of Izaak Neutelings (September 2020)
...
\begin{tikzpicture}
  ...
  \begin{scope}[rotate=0]
    \draw[-{Latex[length=3,width=2,flex'=1]}]
      ...
      node[pos=0.7,above right] {$\vb*{\omega}$};
    \clip (0,0) circle (\E);
 ...
 -- (\ang:\E) coordinate (R);
    \draw[->,thick] (0,-1.2*\E) -- (0,1.6*\E) node[pos=0, below, font=\scriptsize] {Z} node[pos=1.0, above, font=\scriptsize] {N};% the omega axis
    \draw[force, blue!85!black] ($(R)+(\ang:0.075)$) -- ($(R)+(\ang:1)$) node[pos=0.9, above right] {$\vb{v}_0$};
    \draw[->, thick] (O) -- ($(R)+(\ang:0.5)$) node[pos=1,below] {$\vb{z}$};
    \draw[->, thick] ($(R)+(\ang-90:1)$) --  ($(R)+(90+\ang:0.75)$) node[xshift=-3pt, yshift=-4pt] {$\vb{y}$};
    ...
    \draw[gcol,thick] ($(R)+(-0.05,-0.05)$) -- ($(R)+(0.05,0.05)$);
    \draw[gcol,thick] ($(R)+(-0.05,0.05)$) -- ($(R)+(0.05,-0.05)$);

    % node with ω × r in the same color
    \node[gcol,below right=2pt] at ($(R)+(-0.25,0)$) {$\vb*{\omega} \times \vb{r}$};
    ...
    % angle theta on the plane
    \coordinate (RPlane) at ($(R)+(\ang:0.4)$);
    \coordinate (Oplane) at ($(R)+(\ang:0.075)$);
    \coordinate (Aplane) at ($(R)+(0,0.075 + 0.6*\E)$);

    \draw (Oplane) circle (1.5pt);

    \draw pic[<-,"$\theta$"{scale=0.7},
      draw,angle radius=12.5,angle eccentricity=1.3]
      {angle=RPlane--Oplane--Aplane};
