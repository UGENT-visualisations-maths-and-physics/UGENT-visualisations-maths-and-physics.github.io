...
\newcommand{\DrawComplexAxes}[4]{%
  ...
  \draw[arrowstyle, thick]  (-\AxisSizeX/2, 0) -- (\AxisSizeX/2, 0) node[pos=1, right, BigTextFont] {#3};
  \draw[arrowstyle, thick] (0, -\AxisSizeY/2) -- (0, \AxisSizeY/2) node[pos=1, above, BigTextFont] {#4};
}

\DrawComplexAxes{\AreaWidth * 2}{\AreaHeight * 5.0}{$\mathrm{Re}(z)$}{$\mathrm{Im}(z)$}
...
\begin{scope}[xshift = \AreaWidth * 3 * \CmToPt]
  ...
  % draw axes
  \DrawComplexAxes{\AreaWidth * 2}{\AreaWidth * 2}{$u$}{$v$}
