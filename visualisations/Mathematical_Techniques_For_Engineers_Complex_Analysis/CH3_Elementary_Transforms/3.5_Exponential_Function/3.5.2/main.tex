...
\definecolor{BlueBackground}{rgb}{0, 0, 128}
...
\fill[color=BlueBackground, opacity=0.2] (-\AreaWidth, -\AreaHeight) -- (\AreaWidth, -\AreaHeight) -- (\AreaWidth, \AreaHeight) -- (-\AreaWidth, \AreaHeight) -- cycle;
\draw[color=BlueBackground, densely dashed] (-\AreaWidth, -\AreaHeight) -- (\AreaWidth, -\AreaHeight);
\draw[color=BlueBackground] (\AreaWidth, \AreaHeight) -- (-\AreaWidth, \AreaHeight);
...
\newcommand{\DrawComplexAxes}[2]{%
  ...
  \draw[arrowstyle, thick]  (-\AxisSizeX/2, 0) -- (\AxisSizeX/2, 0) node[pos=1, right, BigTextFont] {$\Re(z)$};
  \draw[arrowstyle, thick] (0, -\AxisSizeY/2) -- (0, \AxisSizeY/2) node[pos=1, above, BigTextFont] {$\Im(z)$};
}
...
\begin{scope}[xshift = \AreaWidth * 3 * \CmToPt]
  ...
  \fill[color=BlueBackground, opacity=0.2] (-\BackgroundSize, -\BackgroundSize) rectangle (\BackgroundSize, \BackgroundSize);
  % draw axes
  ...
  \draw[fill=white, draw=black, thick] (0,0) circle [radius=0.08];
\end{scope}
