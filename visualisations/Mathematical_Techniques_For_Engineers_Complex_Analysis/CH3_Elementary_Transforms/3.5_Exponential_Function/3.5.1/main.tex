...
\newcommand{\DrawComplexAxes}[2]{%
  ...
  \begin{pgfonlayer}{foreground}
    \node[ellipse, fill=white, right, inner sep=0pt, font=\scriptsize] 
      at (\AxisSizeX/2, 0) {$\Re(z)$};
    \node[ellipse, fill=white, above, inner sep=0pt, font=\scriptsize] 
      at (0, \AxisSizeY/2) {$\Im(z)$};
  \end{pgfonlayer}
}

\DrawComplexAxes{\AreaWidth * 2}{\AreaHeight * 2}
...
\fill[color=BlueBackground, opacity=0.2] (-\AreaWidth, -\FilledAreaHeight) -- (\AreaWidth, -\FilledAreaHeight) -- (\AreaWidth, \FilledAreaHeight) -- (-\AreaWidth, \FilledAreaHeight) -- cycle;
\draw[color=BlueBackground, densely dashed] (-\AreaWidth, -\FilledAreaHeight) -- (\AreaWidth, -\FilledAreaHeight);
\draw[color=BlueBackground] (\AreaWidth, \FilledAreaHeight) -- (-\AreaWidth, \FilledAreaHeight);
...
\draw[arrowstyle, color=gray, postaction={decorate}, 
    decoration={markings, mark=at position 0.5 with {\node[above, ellipse, fill=white, inner sep=1.5pt] {$w = e^{z}$};}}]
